-- Bind each exam to a deterministic 10-question set per user/course.
-- The same set is returned and graded; no client-provided alternate question IDs are accepted.
...
begin
  ...
  if not (public.can_manage_courses() or public.is_enrolled(p_course_id,v_user)) then
    raise exception 'No tienes acceso a este examen.';
  end if;
  if not public.can_manage_courses() and not public.can_take_exam(p_course_id,v_user) then
  ...
  from (
    ...
    from (
      ...
      order by md5(id::text || ':' || v_user::text || ':' || p_course_id::text)
      limit 10
    ) base
  ) q;
...
begin
  ...
  with expected as (
    ...
    order by md5(q.id::text || ':' || v_user::text || ':' || p_course_id::text)
    limit 10
  )
  select count(*) into v_valid_questions
  from expected e
  where coalesce(p_answers,'{}'::jsonb) ? e.id::text;
  if v_valid_questions <> v_total then
    raise exception 'Las respuestas no corresponden al examen asignado a tu sesión.';
  end if;
  ...
  from expected e
  join public.questions q on q.id=e.id
  join public.question_options o on o.question_id=q.id and o.is_correct=true
  where (coalesce(p_answers,'{}'::jsonb)->>q.id::text)=o.id::text;
